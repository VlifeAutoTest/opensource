package com.jhopesoft.framework.core.dataobject.navigate;
...
import com.jhopesoft.framework.core.dataobjectquery.generate.SqlGenerate;
...
public class NavigateData {
	...
	public JSONObject genJsonObject(SqlGenerate sqlGenerate, FovGridnavigateschemedetail schemeDetail) {
		...
		} else if (schemeDetail.getFDataobjectfield().getFDataobject().getIconcls() != null)
			record.put("iconCls", schemeDetail.getFDataobjectfield().getFDataobject().getIconcls());
		...
		record.put("fieldName", isbasefield ? schemeDetail.getFDataobjectfield().getFieldname()
		...
		if (children != null) {
			...
			for (NavigateData data : children)
				childrens.add(data.genJsonObject(sqlGenerate, schemeDetail));
